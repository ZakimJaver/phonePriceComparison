...
/*
 *  class Call
 *  Stores the phone number and duration of the call
 */
class Call{
private:
    string phoneNumber;
...
/*
 *  class Customer
 *  An abstract class that contains private data members (e.g. cust name, calls made...) that are common to all customers
 */
...
public:
    ...
    void addCall(string number, double duration){
        //Creates a new Call and adds the number and duration in seconds
        Call newCall(number, duration);
        //saves the call in the calls vector
...
public:
    RegularCustomer(){
        //values given by the phone company
        monthlyfee = 5;
        percall = 1;
    }
    ~RegularCustomer(){}
    void computeBalance(){
        ...
        //regular customers balance = monthlyfee + percall × num_calls
 ...
 *  Calls are always rounded up to the next minute
 */
...
/*
 *  class Simulation
 *  randomly generate a large number of customers in each group (between 300 and 400)
 *  Customer:
 *      Randomly generate full customer name
 *      Randomly generate the number of calls a month (between 10 to 300).
 *  Call:
 *      Randomly generate the number called
 *      Randomly generate duration of the call (from 20 seconds to 2 hours)
 * 
 * sort the list of customers (in each group) by their balance (from smallest to highest)
 * 
 * The simulation answers which plan is better for most customers
 */
class Simulation{
...
public:
    ...
    //printing information of each plan and which is better for most customers
    void printResult(){
        ...
        customersRand = (rand() % 100) + 300; //random customers
